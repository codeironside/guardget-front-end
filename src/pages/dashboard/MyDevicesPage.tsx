...
import { Smartphone, Laptop, AlertCircle, Check, X, MoreVertical, Edit, Trash, AlertTriangle } from 'lucide-react';
...
const MyDevicesPage: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'all' | 'reported' | 'missing'>('all');
  const [isLoading, setIsLoading] = useState(true);
  const [deleteModal, setDeleteModal] = useState<{isOpen: boolean, deviceId: string | null}>({
    isOpen: false,
    deviceId: null
  });
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-heading font-bold text-secondary dark:text-white mb-2">My Devices</h1>
        <p className="text-gray-600 dark:text-gray-300">
          Manage all your registered devices. You can have up to 5 devices registered.
        </p>
      </div>
                ...
                >
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between items-start">
                      <div>
                        <h3 className="font-semibold text-secondary dark:text-white">{device.name}</h3>
                        <p className="text-sm text-gray-500 dark:text-gray-400">Model: {device.model}</p>
                      </div>
                    ...
                    <div className="mt-2 space-y-1 text-sm">
                      {device.imei && (
                        <p className="text-gray-600 dark:text-gray-300">IMEI: {device.imei}</p>
                      )}
                      <p className="text-gray-600 dark:text-gray-300">S/N: {device.serialNumber}</p>
                      <p className="text-gray-500 dark:text-gray-400">Added: {device.addedDate}</p>
                    </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 max-w-md w-full mx-4">
            <div className="flex items-start mb-4">
              ...
              <div>
                <h3 className="text-lg font-semibold text-secondary dark:text-white">Delete Device</h3>
                <p className="text-gray-600 dark:text-gray-300 mt-1">
                  This action cannot be undone. The device will be removed from your account after 48 hours.
                </p>
              </div>
            </div>
